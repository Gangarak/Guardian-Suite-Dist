...
# --- DYNAMISCHE PFAD-LOGIK ---
# Ersetzt den starren Marcel-Pfad, damit es bei JEDEM Nutzer läuft
if getattr(sys, 'frozen', False):
    base_path = os.path.dirname(sys.executable)
else:
    # Fallback für deine Entwicklungsumgebung
...
class DashboardWindow(QWidget):
    ...
    def refresh(self):
        ...
        self.gpu_g.set_value(55, alert=(55 > limits['gpu'])) # 55 als Dummy
...
class HUDOverlay(QWidget):
    ...
    def update_stats(self):
        ...
        self.update_label(self.l_gpu, f"GPU: 55°C", 55 > limits['gpu'])
...
class GuardianSuite(QMainWindow):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet("font-size: 28px; color: #00ff99; font-weight: 900;"); header.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.btn_dash.clicked.connect(self.toggle_dashboard); layout.addWidget(self.btn_dash)
        ...
        self.btn_hud.clicked.connect(self.toggle_hud); layout.addWidget(self.btn_hud)
    ...
    def toggle_hud(self):
        if self.hud_overlay is None:
            ...
            self.btn_hud.setText("HUD DEAKTIVIEREN")
            self.btn_hud.setStyleSheet("border: 1px solid #ff4444; padding: 15px; font-weight: bold;")
        else:
            self.hud_overlay.close(); self.hud_overlay = None
            self.btn_hud.setText("HUD OVERLAY AKTIVIEREN")
            self.btn_hud.setStyleSheet("border: 1px solid #00ff99; padding: 15px; font-weight: bold;")

    def check_for_updates(self):
        self.status.setText("Prüfe Version...")
        # Header simulieren einen Browser, damit GitHub die Anfrage nicht blockiert
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if remote_v != CURRENT_VERSION:
                    ...
                    self.status.setText(f"v{CURRENT_VERSION} aktuell.")
            else:
                self.status.setText(f"Fehler: {r.status_code}")
        except:
            self.status.setText("Verbindungsfehler")
